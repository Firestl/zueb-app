...
public class NewHope {
    ...
    public static final boolean STATISTICAL_TEST = false;
    ...
    public static void decodeB(short[] sArr, short[] sArr2, byte[] bArr) {
        ...
        for (int i = 0; i < 256; i++) {
            int i2 = i * 4;
            int i3 = bArr[i + Params.POLY_BYTES] & 255;
    ...
    public static void encodeB(byte[] bArr, short[] sArr, short[] sArr2) {
        ...
        for (int i = 0; i < 256; i++) {
            int i2 = i * 4;
            bArr[i + Params.POLY_BYTES] = (byte) ((sArr2[i2 + 3] << 6) | sArr2[i2] | (sArr2[i2 + 1] << 2) | (sArr2[i2 + 2] << 4));
        }
    }
