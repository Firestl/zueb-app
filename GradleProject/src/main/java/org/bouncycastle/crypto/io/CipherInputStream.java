    ...
    private void finaliseCipher() throws IOException {
        ...
        try {
            ...
            if (this.bufferedBlockCipher != null) {
                iDoFinal = this.bufferedBlockCipher.doFinal(this.buf, 0);
            } else {
                if (this.aeadBlockCipher == null) {
                    this.maxBuf = 0;
                    return;
                }
                iDoFinal = this.aeadBlockCipher.doFinal(this.buf, 0);
            }
    ...
    private int nextChunk() throws IOException {
        ...
        while (true) {
            ...
            try {
                ...
                if (this.bufferedBlockCipher != null) {
                    iProcessBytes = this.bufferedBlockCipher.processBytes(this.inBuf, 0, iProcessBytes, this.buf, 0);
                } else if (this.aeadBlockCipher != null) {
                    iProcessBytes = this.aeadBlockCipher.processBytes(this.inBuf, 0, iProcessBytes, this.buf, 0);
                } else {
