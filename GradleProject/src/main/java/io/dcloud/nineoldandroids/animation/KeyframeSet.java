...
public class KeyframeSet {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.mNumKeyframes; i++) {
            str = String.valueOf(str) + this.mKeyframes.get(i).getValue() + GlideException.IndentedAppendable.INDENT;
        }
        return str;
    }

    /* JADX INFO: renamed from: clone, reason: merged with bridge method [inline-methods] */
    public KeyframeSet m766clone() {
        ArrayList<Keyframe> arrayList = this.mKeyframes;
        int size = arrayList.size();
        Keyframe[] keyframeArr = new Keyframe[size];
        for (int i = 0; i < size; i++) {
            keyframeArr[i] = arrayList.get(i).m765clone();
        }
        return new KeyframeSet(keyframeArr);
    }
}
