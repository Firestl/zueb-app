...
import io.dcloud.media.video.ijkplayer.utils.StringUtils;
...
public class IjkPlayerView extends FrameLayout implements View.OnClickListener {
    ...
    private void _initDanmaku() {
        ...
        NavUtils.getNavigationBarHeight(this.mAttachActivity);
    ...
    private void _initMediaPlayer() {
        ...
        OrientationEventListener orientationEventListener = new OrientationEventListener(this.mAttachActivity) { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.2
    ...
    private void _initView(Context context) {
        ...
        this.mTvReload = (ImageView) findViewById(VideoR.VIDEO_IJK_ID_TV_RELOAD);
        this.mFlReload = findViewById(VideoR.VIDEO_IJK_ID_FL_RELOAD_LAYOUT);
    ...
    private void _loadDanmaku() {
        if (this.mIsEnableDanmaku) {
            ...
            if (this.mDanmakuParser == null) {
                this.mDanmakuParser = new BaseDanmakuParser() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.11
            ...
            try {
                ...
                this.mDanmakuView.setCallback(new DrawHandler.Callback() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.12
    ...
    private void _setControlBarVisible(boolean z) {
        ...
        if (this.mIsNeverPlay) {
            ...
            } else if (this.mIsNeedRecoverScreen) {
                this.mTvRecoverScreen.setVisibility(8);
            }
        }
    }
    ...
    private void _setFullScreen(boolean z) {
        ...
        if (this.mOnPlayerChangedListener != null) {
            this.mOnPlayerChangedListener.onChanged("fullscreenchange", z ? StringUtil.format(this.fullCallFormat, true, Constants.Value.HORIZONTAL) : StringUtil.format(this.fullCallFormat, false, "vertical"));
        }
        ...
        if (i == 3) {
            ...
            if (i == 336) {
                ...
                if (onPlayerChangedListener != null) {
                    onPlayerChangedListener.onChanged("error", "{message:'network error'}");
                    this.mFlReload.setVisibility(0);
                    return;
                }
                return;
    ...
    public IjkPlayerView enableDanmuBtn(boolean z) {
        if (this.mIsEnableDanmaku && z) {
    ...
    public void fullScreen(int i) {
        ...
        if (layoutParams2 instanceof RelativeLayout.LayoutParams) {
            ...
            if (!(layoutParams2 instanceof FrameLayout.LayoutParams)) {
                new AlertDialog.Builder(getContext()).setMessage("nonsupport parent layout, please do it by yourself").setPositiveButton(WXModalUIModule.OK, new DialogInterface.OnClickListener() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.6
    ...
    public void onClick(View view) {
        ...
        if (id == VideoR.VIDEO_IJK_ID_IV_SCREEN_LOCK && this.isShowScreenLockButton) {
    ...
    public void sendDanmaku(JSONObject jSONObject, boolean z) {
        ...
        if (TextUtils.isEmpty(jSONObject.optString("text"))) {
            qv.makeText((Context) this.mAttachActivity, (CharSequence) DCLoudApplicationImpl.self().getContext().getString(R.string.dcloud_feature_media_player_toast_no_content), 0).show();
            return;
        }
        if (!this.mDanmakuView.isPrepared()) {
            qv.makeText((Context) this.mAttachActivity, (CharSequence) DCLoudApplicationImpl.self().getContext().getString(R.string.dcloud_feature_media_player_toast_bullet_not_prepared), 0).show();
            return;
        }
        BaseDanmaku baseDanmakuCreateDanmaku = this.mDanmakuContext.mDanmakuFactory.createDanmaku(this.mDanmakuType);
        if (baseDanmakuCreateDanmaku == null || this.mDanmakuView == null) {
            return;
        }
    ...
    public IjkPlayerView setVideoPath(String str, String str2) {
        return setVideoPath(Uri.parse(str), str2);
    }
    ...
    public IjkPlayerView switchVideoPath(String str, String str2) {
        return switchVideoPath(Uri.parse(str), str2);
    }

    public IjkPlayerView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        this.mHandler = new Handler() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.1
        ...
        this.mSeekListener = new SeekBar.OnSeekBarChangeListener() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.3
            ...
            public void onProgressChanged(SeekBar seekBar, int i, boolean z) {
                ...
                if (z) {
                    ...
                    if (IjkPlayerView.this.mTargetPosition > this.curPosition) {
                        str = StringUtils.generateTime(IjkPlayerView.this.mTargetPosition) + "/" + StringUtils.generateTime(duration) + "\n" + Operators.PLUS + i2 + DCLoudApplicationImpl.self().getContext().getString(R.string.dcloud_feature_media_player_second_unit);
                    } else {
                        str = StringUtils.generateTime(IjkPlayerView.this.mTargetPosition) + "/" + StringUtils.generateTime(duration) + "\n" + i2 + DCLoudApplicationImpl.self().getContext().getString(R.string.dcloud_feature_media_player_second_unit);
                    }
        ...
        this.mHideBarRunnable = new Runnable() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.4
            @Override // java.lang.Runnable
            public void run() {
                IjkPlayerView.this._hideAllView(false);
            }
        };
        this.screenLockHideRunnable = new Runnable() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.5
            @Override // java.lang.Runnable
            public void run() {
        ...
        this.isProgressVisibility = true;
        this.fullCallFormat = "{detail:{fullScreen:%b, direction:'%s'}}";
        this.mOrientation = -90;
        this.defaultSystemUI = 0;
        this.defaultDisplayCutoutMode = 0;
        this.rates = new String[]{"0.5", "0.8", "1.0", "1.25", "1.5"};
        this.mPlayerGestureListener = new GestureDetector.SimpleOnGestureListener() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.7
            ...
            public boolean onSingleTapConfirmed(MotionEvent motionEvent) {
                if (IjkPlayerView.this.isFullscreen()) {
                    ...
                    try {
                        ...
                        jSONObject.put(WXGestureType.GestureInfo.SCREEN_X, motionEvent.getX() / f);
                        jSONObject.put(WXGestureType.GestureInfo.SCREEN_Y, motionEvent.getY() / f);
        ...
        this.mHideTouchViewRunnable = new Runnable() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.8
            @Override // java.lang.Runnable
            public void run() {
                IjkPlayerView.this._hideTouchView();
            }
        };
        this.mPlayerTouchListener = new View.OnTouchListener() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.9
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (this.mode == 1 && !IjkPlayerView.this.mGestureDetector.onTouchEvent(motionEvent) && xa.a(motionEvent) == 1) {
                    IjkPlayerView.this._endGesture();
                }
                return true;
            }
        };
        this.timeUpdateF = "{detail:{currentTime:%f,duration:%f}}";
        this.duration = -1;
        this.isMutePlayer = false;
        this.isLoadingVisibility = true;
        this.mIsRenderingStart = false;
        this.mIsBufferingStart = false;
        this.mInfoListener = new IMediaPlayer.OnInfoListener() { // from class: io.dcloud.media.video.ijkplayer.media.IjkPlayerView.10
    ...
    public IjkPlayerView setVideoPath(Uri uri, String str) {
        this.mVideoView.setVideoURI(uri, str);
        if (uri.toString().startsWith("rtmp:")) {
            ...
            this.mPlayerSeek.setVisibility(this.isProgressVisibility ? 0 : 4);
    ...
    public IjkPlayerView switchVideoPath(Uri uri, String str) {
        reset();
        _setControlBarVisible(true);
        this.duration = -1;
        return setVideoPath(uri, str);
    }

    public IjkPlayerView enableDanmaku(boolean z) {
