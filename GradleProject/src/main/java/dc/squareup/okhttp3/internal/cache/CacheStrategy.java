        ...
        public Factory(long j, Request request, Response response) {
            ...
            if (response != null) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (HttpHeaders.HEAD_KEY_DATE.equalsIgnoreCase(strName)) {
                        this.servedDate = HttpDate.parse(strValue);
                        this.servedDateString = strValue;
                    } else if (HttpHeaders.HEAD_KEY_EXPIRES.equalsIgnoreCase(strName)) {
                        this.expires = HttpDate.parse(strValue);
                    } else if (HttpHeaders.HEAD_KEY_LAST_MODIFIED.equalsIgnoreCase(strName)) {
                        this.lastModified = HttpDate.parse(strValue);
                        this.lastModifiedString = strValue;
                    } else if (HttpHeaders.HEAD_KEY_E_TAG.equalsIgnoreCase(strName)) {
                        this.etag = strValue;
                    } else if ("Age".equalsIgnoreCase(strName)) {
                        this.ageSeconds = dc.squareup.okhttp3.internal.http.HttpHeaders.parseSeconds(strValue, -1);
                    }
                }
            }
        }
