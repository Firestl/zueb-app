...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    public static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable("BufferGifDecoder", 2) && iMax > 1) {
            Log.v("BufferGifDecoder", "Downsampling GIF, sampleSize: " + iMax + ", target dimens: [" + i + Constants.Name.X + i2 + "], actual dimens: [" + gifHeader.getWidth() + Constants.Name.X + gifHeader.getHeight() + Operators.ARRAY_END_STR);
        }
        return iMax;
    }
