package supwisdom;

import android.database.CursorWindow;
import android.os.Bundle;
import android.os.Parcel;
import android.os.Parcelable;
import com.google.android.gms.common.data.DataHolder;
import com.google.android.gms.common.internal.safeparcel.SafeParcelReader;

/* JADX INFO: compiled from: com.google.android.gms:play-services-base@@17.5.0 */
/* JADX INFO: loaded from: classes.dex */
public final class gf0 implements Parcelable.Creator<DataHolder> {
    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ DataHolder createFromParcel(Parcel parcel) {
        int iB = SafeParcelReader.b(parcel);
        String[] strArrO = null;
        CursorWindow[] cursorWindowArr = null;
        Bundle bundleF = null;
        int iV = 0;
        int iV2 = 0;
        while (parcel.dataPosition() < iB) {
            int iA = SafeParcelReader.a(parcel);
            int iA2 = SafeParcelReader.a(iA);
            if (iA2 == 1) {
                strArrO = SafeParcelReader.o(parcel, iA);
            } else if (iA2 == 2) {
                cursorWindowArr = (CursorWindow[]) SafeParcelReader.b(parcel, iA, CursorWindow.CREATOR);
            } else if (iA2 == 3) {
                iV2 = SafeParcelReader.v(parcel, iA);
            } else if (iA2 == 4) {
                bundleF = SafeParcelReader.f(parcel, iA);
            } else if (iA2 != 1000) {
                SafeParcelReader.y(parcel, iA);
            } else {
                iV = SafeParcelReader.v(parcel, iA);
            }
        }
        SafeParcelReader.q(parcel, iB);
        DataHolder dataHolder = new DataHolder(iV, strArrO, cursorWindowArr, iV2, bundleF);
        dataHolder.e();
        return dataHolder;
    }

    @Override // android.os.Parcelable.Creator
    public final /* synthetic */ DataHolder[] newArray(int i) {
        return new DataHolder[i];
    }
}
