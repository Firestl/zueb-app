package supwisdom;

import androidx.recyclerview.widget.RecyclerView;
import com.taobao.weex.el.parse.Operators;
import java.util.ArrayList;
import java.util.List;
import supwisdom.lf;

/* JADX INFO: compiled from: AdapterHelper.java */
/* JADX INFO: loaded from: classes.dex */
public class af implements lf.a {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    public ka<b> f6892a;
    public final ArrayList<b> b;
    public final ArrayList<b> c;
    public final a d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    public Runnable f6893e;
    public final boolean f;
    public final lf g;
    public int h;

    /* JADX INFO: compiled from: AdapterHelper.java */
    public interface a {
        RecyclerView.b0 a(int i);

        void a(int i, int i2);

        void a(int i, int i2, Object obj);

        void a(b bVar);

        void b(int i, int i2);

        void b(b bVar);

        void c(int i, int i2);

        void d(int i, int i2);
    }

    /* JADX INFO: compiled from: AdapterHelper.java */
    public static class b {

        /* JADX INFO: renamed from: a, reason: collision with root package name */
        public int f6894a;
        public int b;
        public Object c;
        public int d;

        public b(int i, int i2, int i3, Object obj) {
            this.f6894a = i;
            this.b = i2;
            this.d = i3;
            this.c = obj;
        }

        public String a() {
            int i = this.f6894a;
            return i != 1 ? i != 2 ? i != 4 ? i != 8 ? "??" : "mv" : "up" : "rm" : "add";
        }

        public boolean equals(Object obj) {
            if (this == obj) {
                return true;
            }
            if (obj == null || b.class != obj.getClass()) {
                return false;
            }
            b bVar = (b) obj;
            int i = this.f6894a;
            if (i != bVar.f6894a) {
                return false;
            }
            if (i == 8 && Math.abs(this.d - this.b) == 1 && this.d == bVar.b && this.b == bVar.d) {
                return true;
            }
            if (this.d != bVar.d || this.b != bVar.b) {
                return false;
            }
            Object obj2 = this.c;
            if (obj2 != null) {
                if (!obj2.equals(bVar.c)) {
                    return false;
                }
            } else if (bVar.c != null) {
                return false;
            }
            return true;
        }

        public int hashCode() {
            return (((this.f6894a * 31) + this.b) * 31) + this.d;
        }

        public String toString() {
            return Integer.toHexString(System.identityHashCode(this)) + Operators.ARRAY_START_STR + a() + ",s:" + this.b + "c:" + this.d + ",p:" + this.c + Operators.ARRAY_END_STR;
        }
    }

    public af(a aVar) {
        this(aVar, false);
    }

    public void a() {
        int size = this.c.size();
        for (int i = 0; i < size; i++) {
            this.d.b(this.c.get(i));
        }
        a(this.c);
        this.h = 0;
    }

    public final boolean b(int i) {
        int size = this.c.size();
        for (int i2 = 0; i2 < size; i2++) {
            b bVar = this.c.get(i2);
            int i3 = bVar.f6894a;
            if (i3 == 8) {
                if (a(bVar.d, i2 + 1) == i) {
                    return true;
                }
            } else if (i3 == 1) {
                int i4 = bVar.b;
                int i5 = bVar.d + i4;
                while (i4 < i5) {
                    if (a(i4, i2 + 1) == i) {
                        return true;
                    }
                    i4++;
                }
            } else {
                continue;
            }
        }
        return false;
    }

    public final void c(b bVar) {
        g(bVar);
    }

    public final void d(b bVar) {
        boolean z;
        byte b2;
        int i = bVar.b;
        int i2 = bVar.d + i;
        byte b3 = -1;
        int i3 = i;
        int i4 = 0;
        while (i3 < i2) {
            if (this.d.a(i3) != null || b(i3)) {
                if (b3 == 0) {
                    f(a(2, i, i4, null));
                    z = true;
                } else {
                    z = false;
                }
                b2 = 1;
            } else {
                if (b3 == 1) {
                    g(a(2, i, i4, null));
                    z = true;
                } else {
                    z = false;
                }
                b2 = 0;
            }
            if (z) {
                i3 -= i4;
                i2 -= i4;
                i4 = 1;
            } else {
                i4++;
            }
            i3++;
            b3 = b2;
        }
        if (i4 != bVar.d) {
            a(bVar);
            bVar = a(2, i, i4, null);
        }
        if (b3 == 0) {
            f(bVar);
        } else {
            g(bVar);
        }
    }

    public void e() {
        this.g.b(this.b);
        int size = this.b.size();
        for (int i = 0; i < size; i++) {
            b bVar = this.b.get(i);
            int i2 = bVar.f6894a;
            if (i2 == 1) {
                b(bVar);
            } else if (i2 == 2) {
                d(bVar);
            } else if (i2 == 4) {
                e(bVar);
            } else if (i2 == 8) {
                c(bVar);
            }
            Runnable runnable = this.f6893e;
            if (runnable != null) {
                runnable.run();
            }
        }
        this.b.clear();
    }

    public void f() {
        a(this.b);
        a(this.c);
        this.h = 0;
    }

    public final void g(b bVar) {
        this.c.add(bVar);
        int i = bVar.f6894a;
        if (i == 1) {
            this.d.c(bVar.b, bVar.d);
            return;
        }
        if (i == 2) {
            this.d.b(bVar.b, bVar.d);
            return;
        }
        if (i == 4) {
            this.d.a(bVar.b, bVar.d, bVar.c);
        } else {
            if (i == 8) {
                this.d.a(bVar.b, bVar.d);
                return;
            }
            throw new IllegalArgumentException("Unknown update op type for " + bVar);
        }
    }

    public af(a aVar, boolean z) {
        this.f6892a = new la(30);
        this.b = new ArrayList<>();
        this.c = new ArrayList<>();
        this.h = 0;
        this.d = aVar;
        this.f = z;
        this.g = new lf(this);
    }

    public boolean c() {
        return this.b.size() > 0;
    }

    public int c(int i) {
        return a(i, 0);
    }

    public boolean c(int i, int i2) {
        if (i2 < 1) {
            return false;
        }
        this.b.add(a(2, i, i2, null));
        this.h |= 2;
        return this.b.size() == 1;
    }

    public final void f(b bVar) {
        int i;
        int i2 = bVar.f6894a;
        if (i2 != 1 && i2 != 8) {
            int iD = d(bVar.b, i2);
            int i3 = bVar.b;
            int i4 = bVar.f6894a;
            if (i4 == 2) {
                i = 0;
            } else {
                if (i4 != 4) {
                    throw new IllegalArgumentException("op should be remove or update." + bVar);
                }
                i = 1;
            }
            int i5 = 1;
            for (int i6 = 1; i6 < bVar.d; i6++) {
                int iD2 = d(bVar.b + (i * i6), bVar.f6894a);
                int i7 = bVar.f6894a;
                if (i7 == 2 ? iD2 == iD : i7 == 4 && iD2 == iD + 1) {
                    i5++;
                } else {
                    b bVarA = a(bVar.f6894a, iD, i5, bVar.c);
                    a(bVarA, i3);
                    a(bVarA);
                    if (bVar.f6894a == 4) {
                        i3 += i5;
                    }
                    iD = iD2;
                    i5 = 1;
                }
            }
            Object obj = bVar.c;
            a(bVar);
            if (i5 > 0) {
                b bVarA2 = a(bVar.f6894a, iD, i5, obj);
                a(bVarA2, i3);
                a(bVarA2);
                return;
            }
            return;
        }
        throw new IllegalArgumentException("should not dispatch add or move for pre layout");
    }

    public void a(b bVar, int i) {
        this.d.a(bVar);
        int i2 = bVar.f6894a;
        if (i2 == 2) {
            this.d.d(i, bVar.d);
        } else {
            if (i2 == 4) {
                this.d.a(i, bVar.d, bVar.c);
                return;
            }
            throw new IllegalArgumentException("only remove and update ops can be dispatched in first pass");
        }
    }

    public final void b(b bVar) {
        g(bVar);
    }

    public boolean b(int i, int i2) {
        if (i2 < 1) {
            return false;
        }
        this.b.add(a(1, i, i2, null));
        this.h |= 1;
        return this.b.size() == 1;
    }

    public int a(int i, int i2) {
        int size = this.c.size();
        while (i2 < size) {
            b bVar = this.c.get(i2);
            int i3 = bVar.f6894a;
            if (i3 == 8) {
                int i4 = bVar.b;
                if (i4 == i) {
                    i = bVar.d;
                } else {
                    if (i4 < i) {
                        i--;
                    }
                    if (bVar.d <= i) {
                        i++;
                    }
                }
            } else {
                int i5 = bVar.b;
                if (i5 > i) {
                    continue;
                } else if (i3 == 2) {
                    int i6 = bVar.d;
                    if (i < i5 + i6) {
                        return -1;
                    }
                    i -= i6;
                } else if (i3 == 1) {
                    i += bVar.d;
                }
            }
            i2++;
        }
        return i;
    }

    public void b() {
        a();
        int size = this.b.size();
        for (int i = 0; i < size; i++) {
            b bVar = this.b.get(i);
            int i2 = bVar.f6894a;
            if (i2 == 1) {
                this.d.b(bVar);
                this.d.c(bVar.b, bVar.d);
            } else if (i2 == 2) {
                this.d.b(bVar);
                this.d.d(bVar.b, bVar.d);
            } else if (i2 == 4) {
                this.d.b(bVar);
                this.d.a(bVar.b, bVar.d, bVar.c);
            } else if (i2 == 8) {
                this.d.b(bVar);
                this.d.a(bVar.b, bVar.d);
            }
            Runnable runnable = this.f6893e;
            if (runnable != null) {
                runnable.run();
            }
        }
        a(this.b);
        this.h = 0;
    }

    public final void e(b bVar) {
        int i = bVar.b;
        int i2 = bVar.d + i;
        int i3 = i;
        byte b2 = -1;
        int i4 = 0;
        while (i < i2) {
            if (this.d.a(i) != null || b(i)) {
                if (b2 == 0) {
                    f(a(4, i3, i4, bVar.c));
                    i3 = i;
                    i4 = 0;
                }
                b2 = 1;
            } else {
                if (b2 == 1) {
                    g(a(4, i3, i4, bVar.c));
                    i3 = i;
                    i4 = 0;
                }
                b2 = 0;
            }
            i4++;
            i++;
        }
        if (i4 != bVar.d) {
            Object obj = bVar.c;
            a(bVar);
            bVar = a(4, i3, i4, obj);
        }
        if (b2 == 0) {
            f(bVar);
        } else {
            g(bVar);
        }
    }

    public final int d(int i, int i2) {
        for (int size = this.c.size() - 1; size >= 0; size--) {
            b bVar = this.c.get(size);
            int i3 = bVar.f6894a;
            if (i3 == 8) {
                int i4 = bVar.b;
                int i5 = bVar.d;
                if (i4 >= i5) {
                    i5 = i4;
                    i4 = i5;
                }
                if (i >= i4 && i <= i5) {
                    int i6 = bVar.b;
                    if (i4 == i6) {
                        if (i2 == 1) {
                            bVar.d++;
                        } else if (i2 == 2) {
                            bVar.d--;
                        }
                        i++;
                    } else {
                        if (i2 == 1) {
                            bVar.b = i6 + 1;
                        } else if (i2 == 2) {
                            bVar.b = i6 - 1;
                        }
                        i--;
                    }
                } else {
                    int i7 = bVar.b;
                    if (i < i7) {
                        if (i2 == 1) {
                            bVar.b = i7 + 1;
                            bVar.d++;
                        } else if (i2 == 2) {
                            bVar.b = i7 - 1;
                            bVar.d--;
                        }
                    }
                }
            } else {
                int i8 = bVar.b;
                if (i8 <= i) {
                    if (i3 == 1) {
                        i -= bVar.d;
                    } else if (i3 == 2) {
                        i += bVar.d;
                    }
                } else if (i2 == 1) {
                    bVar.b = i8 + 1;
                } else if (i2 == 2) {
                    bVar.b = i8 - 1;
                }
            }
        }
        for (int size2 = this.c.size() - 1; size2 >= 0; size2--) {
            b bVar2 = this.c.get(size2);
            if (bVar2.f6894a == 8) {
                int i9 = bVar2.d;
                if (i9 == bVar2.b || i9 < 0) {
                    this.c.remove(size2);
                    a(bVar2);
                }
            } else if (bVar2.d <= 0) {
                this.c.remove(size2);
                a(bVar2);
            }
        }
        return i;
    }

    public boolean a(int i, int i2, Object obj) {
        if (i2 < 1) {
            return false;
        }
        this.b.add(a(4, i, i2, obj));
        this.h |= 4;
        return this.b.size() == 1;
    }

    public boolean a(int i, int i2, int i3) {
        if (i == i2) {
            return false;
        }
        if (i3 == 1) {
            this.b.add(a(8, i, i2, null));
            this.h |= 8;
            return this.b.size() == 1;
        }
        throw new IllegalArgumentException("Moving more than 1 item is not supported yet");
    }

    public int a(int i) {
        int size = this.b.size();
        for (int i2 = 0; i2 < size; i2++) {
            b bVar = this.b.get(i2);
            int i3 = bVar.f6894a;
            if (i3 != 1) {
                if (i3 == 2) {
                    int i4 = bVar.b;
                    if (i4 <= i) {
                        int i5 = bVar.d;
                        if (i4 + i5 > i) {
                            return -1;
                        }
                        i -= i5;
                    } else {
                        continue;
                    }
                } else if (i3 == 8) {
                    int i6 = bVar.b;
                    if (i6 == i) {
                        i = bVar.d;
                    } else {
                        if (i6 < i) {
                            i--;
                        }
                        if (bVar.d <= i) {
                            i++;
                        }
                    }
                }
            } else if (bVar.b <= i) {
                i += bVar.d;
            }
        }
        return i;
    }

    @Override // supwisdom.lf.a
    public b a(int i, int i2, int i3, Object obj) {
        b bVarAcquire = this.f6892a.acquire();
        if (bVarAcquire == null) {
            return new b(i, i2, i3, obj);
        }
        bVarAcquire.f6894a = i;
        bVarAcquire.b = i2;
        bVarAcquire.d = i3;
        bVarAcquire.c = obj;
        return bVarAcquire;
    }

    @Override // supwisdom.lf.a
    public void a(b bVar) {
        if (this.f) {
            return;
        }
        bVar.c = null;
        this.f6892a.release(bVar);
    }

    public boolean d(int i) {
        return (i & this.h) != 0;
    }

    public boolean d() {
        return (this.c.isEmpty() || this.b.isEmpty()) ? false : true;
    }

    public void a(List<b> list) {
        int size = list.size();
        for (int i = 0; i < size; i++) {
            a(list.get(i));
        }
        list.clear();
    }
}
